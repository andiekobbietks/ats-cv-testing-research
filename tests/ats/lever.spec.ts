...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'Lever'
);

if (!atsSystem) {
  throw new Error('Lever ATS system not found in ats-systems.json');
}

test.describe('Lever ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `lever-${format}-${Date.now()}`;
