...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'Recruitee'
);

if (!atsSystem) {
  throw new Error('Recruitee ATS system not found in ats-systems.json');
}

test.describe('Recruitee ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `recruitee-${format}-${Date.now()}`;
