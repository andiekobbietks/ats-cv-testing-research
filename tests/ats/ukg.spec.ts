...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'UKG'
);

if (!atsSystem) {
  throw new Error('UKG ATS system not found in ats-systems.json');
}

test.describe('UKG ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `ukg-${format}-${Date.now()}`;
