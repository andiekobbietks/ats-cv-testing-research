...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'SEEK Talent'
);

if (!atsSystem) {
  throw new Error('SEEK Talent ATS system not found in ats-systems.json');
}

test.describe('SEEK Talent ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `seek-talent-${format}-${Date.now()}`;
    ...
    try {
      ...
      const compileResult = await latexCompiler.compile(
        cvResponse.latex,
        `cv-seek-talent-${format}-${Date.now()}`,
        { cleanup: true }
      );
