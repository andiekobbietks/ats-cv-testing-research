...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'Oracle Taleo'
);

if (!atsSystem) {
  throw new Error('Oracle Taleo ATS system not found in ats-systems.json');
}

test.describe('Oracle Taleo ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `oracle-taleo-${format}-${Date.now()}`;
    ...
    try {
      ...
      const compileResult = await latexCompiler.compile(
        cvResponse.latex,
        `cv-oracle-taleo-${format}-${Date.now()}`,
        { cleanup: true }
      );
