...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'SAP SuccessFactors'
);

if (!atsSystem) {
  throw new Error('SAP SuccessFactors ATS system not found in ats-systems.json');
}

test.describe('SAP SuccessFactors ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `sap-successfactors-${format}-${Date.now()}`;
    ...
    try {
      ...
      const compileResult = await latexCompiler.compile(
        cvResponse.latex,
        `cv-sap-successfactors-${format}-${Date.now()}`,
        { cleanup: true }
      );
