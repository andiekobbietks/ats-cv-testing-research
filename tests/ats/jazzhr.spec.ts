...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'JazzHR'
);

if (!atsSystem) {
  throw new Error('JazzHR ATS system not found in ats-systems.json');
}

test.describe('JazzHR ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `jazzhr-${format}-${Date.now()}`;
