...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'Greenhouse'
);

if (!atsSystem) {
  throw new Error('Greenhouse ATS system not found in ats-systems.json');
}

test.describe('Greenhouse ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `greenhouse-${format}-${Date.now()}`;
