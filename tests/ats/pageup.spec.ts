...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'PageUp'
);

if (!atsSystem) {
  throw new Error('PageUp ATS system not found in ats-systems.json');
}

test.describe('PageUp ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `pageup-${format}-${Date.now()}`;
