...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'Teamtailor'
);

if (!atsSystem) {
  throw new Error('Teamtailor ATS system not found in ats-systems.json');
}

test.describe('Teamtailor ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `teamtailor-${format}-${Date.now()}`;
