...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'iCIMS'
);

if (!atsSystem) {
  throw new Error('iCIMS ATS system not found in ats-systems.json');
}

test.describe('iCIMS ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `icims-${format}-${Date.now()}`;
