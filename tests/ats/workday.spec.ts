...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'Workday'
);

if (!atsSystem) {
  throw new Error('Workday ATS system not found in ats-systems.json');
}

test.describe('Workday ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `workday-${format}-${Date.now()}`;
