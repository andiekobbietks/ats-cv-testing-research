...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'SmartRecruiters'
);

if (!atsSystem) {
  throw new Error('SmartRecruiters ATS system not found in ats-systems.json');
}

test.describe('SmartRecruiters ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `smartrecruiters-${format}-${Date.now()}`;
    ...
    try {
      ...
      const compileResult = await latexCompiler.compile(
        cvResponse.latex,
        `cv-smartrecruiters-${format}-${Date.now()}`,
        { cleanup: true }
      );
