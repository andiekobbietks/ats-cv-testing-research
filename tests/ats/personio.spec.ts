...
const atsSystem = [...atsSystemsData.tier1, ...atsSystemsData.tier2].find(
  (system) => system.name === 'Personio'
);

if (!atsSystem) {
  throw new Error('Personio ATS system not found in ats-systems.json');
}

test.describe('Personio ATS Tests', () => {
  ...
  async function testATSParsing(
    ...
    const testId = `personio-${format}-${Date.now()}`;
